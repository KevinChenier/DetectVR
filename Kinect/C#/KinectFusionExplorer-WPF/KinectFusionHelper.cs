...
namespace Microsoft.Samples.Kinect.KinectFusionExplorer
{
    ...
    public static class Helper
    {
        ...
        public static string SaveObjFileStringForUnity(ColorMesh mesh, bool flipAxes)
        {
            if (null == mesh)
            {
                return null;
            ...
            StringBuilder ObjFileToString = new StringBuilder();
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                ObjFileToString.Append(vertexString);
            }

            ObjFileToString.Append('@');

            for (int i = 0; i < normals.Count; i++)
            {
                var normal = normals[i];

                string normalString = normal.X.ToString(CultureInfo.InvariantCulture) + ",";
                ...
                ObjFileToString.Append(normalString);
            }
            ...
            for (int i = 0; i < indices.Count; i++)
            {
                ...
                string indiceString = indice.ToString(CultureInfo.InvariantCulture);

                if (!(i == indices.Count - 1))
                    indiceString += ",";

                ObjFileToString.Append(indiceString);
            }
            return ObjFileToString.ToString();
        }
